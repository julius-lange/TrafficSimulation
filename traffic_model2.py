...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  4 15:06:49 2022
...
def path_sensitivity(origin,destination,G,edges,times):
    ...
    #time_ideal=ideal_time(path_ideal,G)
    ...
    for i in path_edges:
        ...
        local_ideal_time=ideal_time([i[0], i[1]],G)
        local_actual_time=path_time(G,edges,times,[i[0], i[1]])
        #augmented_path_costs.append(100)
        #augmented_path_costs.append(2*ideal_path_costs[j])
        augmented_path_costs.append(ideal_path_costs[j]*local_actual_time/local_ideal_time*0.8)
    ...
    if alternative_path==path_ideal:
...
def path_sensitivites(G,origins,destinations,passengers,paths,times):
    ...
    for i in range(len(origins)):
        print(i)
        sensitivty,best_alternative=path_sensitivity(origins[i],destinations[i],G,edges,times)
